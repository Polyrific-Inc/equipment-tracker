...
TEST_F(TimestampUtilsTest, TimestampDiffDays) {
    ...
    Timestamp t4 = parseTimestamp("2023-05-14 00:00:00", "%Y-%m-%d %H:%M:%S");
    EXPECT_EQ(timestampDiffDays(t3, t4), 1); // Should be 1.5 days, but integer division gives 1
}
...
TEST_F(TimestampUtilsTest, AddHours) {
    ...
    Timestamp t4 = addHours(t1, 0);
    EXPECT_EQ(formatTimestamp(t4, "%Y-%m-%d %H:%M:%S"), "2023-05-15 10:30:45");
    
    // Test day boundary
    Timestamp t5 = addHours(t1, 24);
    EXPECT_EQ(formatTimestamp(t5, "%Y-%m-%d %H:%M:%S"), "2023-05-16 10:30:45");
}
...
TEST_F(TimestampUtilsTest, AddDays) {
    ...
    Timestamp t4 = addDays(t1, 0);
    EXPECT_EQ(formatTimestamp(t4, "%Y-%m-%d %H:%M:%S"), "2023-05-15 10:30:45");
    
    // Test month boundary
    Timestamp t5 = addDays(t1, 20);
    EXPECT_EQ(formatTimestamp(t5, "%Y-%m-%d %H:%M:%S"), "2023-06-04 10:30:45");
}
